...
public class OutHouseServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		// 把数据写入到数据库中入库表中
		response.sendRedirect("outhouse/outgoods.jsp");
...
//				request, response);
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (null == inlistcode) {
			...
			inlistcode = "ckd-" + df2list.format(new Date());
		...
		request.getRequestDispatcher("outhouse/outgoods.jsp").forward(
		request, response);
	}
}
