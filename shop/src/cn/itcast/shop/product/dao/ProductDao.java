...
public class ProductDao extends HibernateDaoSupport {
	...
	public int findCsid(String csname) {
			String hql = "select csid from CategorySecond where csname = ?";
			List<Integer> list = this.getHibernateTemplate().find(hql, csname);
			if(list != null && list.size() > 0){
				return list.get(0).intValue();
			}
			return 0;
	...
	public List<Product> adminfindByPageCsid(Integer csid, int begin, int limit) {
		String hql = "select p from Product p join p.categorySecond cs where cs.csid = ?";
		List<Product> list = this.getHibernateTemplate().execute(new PageHibernateCallback<Product>(hql, new Object[]{csid}, begin, limit));
		if(list != null && list.size() > 0){
			return list;
